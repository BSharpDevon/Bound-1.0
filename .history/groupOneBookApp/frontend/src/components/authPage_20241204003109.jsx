...
function AuthPage() {
  ...
  const genres = ["Mystery", "Fantasy", "Romance", "Science Fiction", "Thriller"];
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      // Handle transitions
      if (!isDeleting && charIndex === currentGenre.length) {
        setTimeout(() => setIsDeleting(true), 1000); // Pause before deleting
      } else if (isDeleting && charIndex === 0) {
        setIsDeleting(false);
        setGenreIndex((prev) => (prev + 1) % genres.length); // Move to next genre
      }
    }, typingSpeed);

    return () => clearTimeout(timeout); // Clean up timeout
  }, [charIndex, isDeleting, genres, genreIndex]);

  return (
    <h1>
      Let Your Friends Find Your Next Best{" "}
      <span className="typewriter">
        {genres[genreIndex].substring(0, charIndex)}
      </span>
      <span className="cursor"></span> Book
    </h1>
  );
};
    ...
    <div className="auth-page">
      ...
      <div className="intro-section">
        <div id="introMessage">
          ...
          <p>
Discover the perfect book. Bound searches millions of titles to match you and your friend’s unique tastes. Get a magical recommendation made just for you.</p>
